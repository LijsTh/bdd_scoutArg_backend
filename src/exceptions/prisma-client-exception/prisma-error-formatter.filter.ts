...
export class PrismaErrorFormatter {
  ...
  getFriendlyMessage(): string {
    switch (this.exception.code) {
      case 'P2001': {
        const { model_name, argument_name, argument_value } =
          this.exception.meta || {};
      ...
      case 'P2004': {
        const database_error =
          this.exception.meta?.database_error || 'unknown database error';
      ...
      case 'P2011': {
        const constraint =
          this.exception.meta?.constraint || 'unknown constraint';
      ...
      case 'P2013': {
        const { argument_name, field_name, object_name } =
          this.exception.meta || {};
        return `Missing the required argument '${argument_name}' for field '${field_name}' on '${object_name}'.`;
      }
      case 'P2014': {
        const { relation_name, model_a_name, model_b_name } =
          this.exception.meta || {};
      ...
      case 'P2016': {
        const details =
          this.exception.meta?.details || 'query interpretation error';
        return `Query interpretation error. ${details}`;
      }
      case 'P2017': {
        const relation_name =
          this.exception.meta?.relation_name || 'unknown relation';
        return `The records for relation '${relation_name}' are not connected.`;
      }
      case 'P2018': {
        const details =
          this.exception.meta?.details ||
      ...
      case 'P2023': {
        const message =
          this.exception.meta?.message || 'inconsistent column data';
      ...
      case 'P2029': {
        const message =
          this.exception.meta?.message || 'query parameter limit exceeded';
      ...
      case 'P2035': {
        const database_error =
          this.exception.meta?.database_error || 'assertion violation';
